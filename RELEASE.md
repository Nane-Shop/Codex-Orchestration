...
5. If Claude Fable 5 is included in the release, verify both supported seat paths from a first-party Claude login: Fable Planner `create_plan`/`revise_plan` with a different Advisor, and Fable Advisor `review_plan` with root planning. Confirm the pinned primary model, exact allowlisted helper set reported by runtime metadata, effort, status, the bounded approval loop, and disable/restore. Derive timeout arithmetic from the packaged constants and configuration: each MCP tool timeout must exceed `CLAUDE_TIMEOUT_SECONDS + PROCESS_TEARDOWN_RESERVE_SECONDS` (currently `600 > 570 + 20`), while `PROCESS_TEARDOWN_DEADLINE_SECONDS` remains strictly inside that reserve (currently 19 seconds). Every Python iteration and external teardown operation must consume that one deadline rather than a per-operation allowance. An unknown helper model is a release failure, not an implicit allowlist expansion.
6. If Claude Opus 5 is included in the release, separately qualify both live seat paths from a first-party Claude login. Run Opus Planner `create_plan` and force a reviewed `PLAN_REVISE` round through `revise_plan` with a distinct Advisor; then run Opus Advisor `review_plan` with root planning. For both paths confirm the pinned `claude-opus-5` primary, exact ten-key `canonicalModel=claude-opus-5` and `provider=firstParty` runtime identity, primary-only policy, configured effort, no tools/session persistence, `status --require-effective`, the five-attempt approval bound, and disable/restore. Schema 3 is cumulative: include the schema-2 `runtime_probe` record and bind `opus_runtime_qualification` to the exact reviewed head SHA with Claude Code version, configured/canonical model, provider, effort, status, and specific live evidence. Both records are mandatory when OpenRouter and Advisor bridge paths change together. Any helper or other runtime model is a release failure. A local fake-model pass or configured route is readiness only and never live Opus proof; local preflight must never invoke Opus.
