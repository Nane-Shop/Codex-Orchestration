...
class PackagingTests(unittest.TestCase):
    ...
    def test_canonical_marketplace_source_is_used_in_operator_contracts(self) -> None:
        ...
        self.assertIn("## 0.11.0 — 2026-08-12", changelog)
    ...
    def test_opus_is_the_leading_bundled_advisor_prompt(self) -> None:
        manifest = json.loads(
            (PLUGIN_ROOT / ".codex-plugin" / "plugin.json").read_text(
                encoding="utf-8"
            )
        ...
        prompts = manifest["interface"]["defaultPrompt"]
        self.assertIn("Advisor: Claude Opus 5 High", prompts[0])

        metadata = (SKILL_ROOT / "agents" / "openai.yaml").read_text(
            encoding="utf-8"
        )
        self.assertIn("advisor: Claude Opus 5 High", metadata)

        readme = (REPO_ROOT / "README.md").read_text(encoding="utf-8")
        self.assertLess(
            readme.index("setup advisor: Claude Opus 5"),
            readme.index("setup advisor: Claude Fable 5"),
        )
    ...
    def test_ascii_and_role_copy_are_plain_and_root_centered(self) -> None:
        ...
        self.assertIn("ADVISOR REVIEWS ONCE", readme)
        self.assertIn("REVIEWER REVIEWS ONCE", readme)
    ...
    def test_advisor_protocol_is_bounded_and_root_only(self) -> None:
        ...
        self.assertIn("five rounds and five attempts as a transport/session", skill)
    ...
    def test_opus_leads_quick_start_and_fable_remains_available(self) -> None:
        readme = (REPO_ROOT / "README.md").read_text(encoding="utf-8")
